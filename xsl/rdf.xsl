...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bk="http://gams.uni-graz.at/rem/bookkeeping/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:g2o="http://gams.uni-graz.at/onto/#" xmlns:gl="http://www.xbrl.org/GLTaxonomy/" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:tei="http://www.tei-c.org/ns/1.0" version="2.0">
    
    
    <xsl:include href="http://gams.uni-graz.at/archive/objects/cirilo:srbas/datastreams/STYLESHEET.CONVERSIONS/content" />
    ...
    <xsl:variable name="base-uri" select="concat('http://gams.uni-graz.at/archive/get/',$pid,'/sdef:TEI/get#')" />
    <xsl:variable name="accounts" select="//tei:taxonomy[@ana='#bk_account']" />
    <xsl:variable name="xmlBaseAccounts"><xsl:choose>
        <xsl:when test="//tei:classDecl[.//tei:taxonomy/@ana='#bk_account']/starts-with(@xml:base,'http://')"><xsl:value-of select="//tei:classDecl[.//tei:taxonomy/@ana='#bk_account']/@xml:base"/></xsl:when>
        <xsl:otherwise>http://gams.uni-graz.at<xsl:value-of select="//tei:classDecl[.//tei:taxonomy/@ana='#bk_account']/@xml:base"/></xsl:otherwise>
    </xsl:choose> 
    </xsl:variable>
    ...
    <xsl:template match="/">
        <rdf:RDF>
            <rdf:Description rdf:about="http://gams.uni-graz.at/{$pid}">
                <g2o:sameAs rdf:resource="info:fedora/{/tei:TEI/tei:teiHeader/tei:fileDesc/tei:publicationStmt/tei:idno[@type='PID']/text()}"/>
                <tei:msIdentifier>
                    <xsl:value-of select="//tei:msIdentifier" />
                </tei:msIdentifier>
                <dc:date>
                    <xsl:attribute name="rdf:datatype">http://www.w3.org/2001/XMLSchema#int</xsl:attribute>
                    <xsl:value-of select="//tei:teiHeader//tei:sourceDesc//tei:origDate[1]/substring-before(@to,'-')" />
                </dc:date>
            </rdf:Description>
            
            <xsl:apply-templates select="//tei:taxonomy[matches(@ana,'#bk_account') or matches(@ana,'#gl_account')]" />
            <xsl:apply-templates select="//tei:*[matches(@ana,'#bk_entry') or matches(@ana,'#gl_entryDetail') or matches(@ana, '#bk_total')]" />
    ...
    <xsl:template match="*[substring-before(@ana,'_')='#bk']|*[substring-before(@ana,'_')='#gl']" priority="-2">
        ...
        <rdf:Description>
            ...
            <rdf:type>
                <xsl:attribute name="rdf:resource" select="replace(./@ana,'bk:','http://gams.uni-graz.at/rem/bookkeeping/#')" />
            </rdf:type>
            ...
            <xsl:apply-templates select="./tei:measure[./@type='currency'] | .//tei:*[@ana='#bk_amount']" />
            <bk:inhalt>
    ...
    <xsl:template match="//tei:taxonomy[@ana='#bk_account']">
        ...
        <!-- ToDo: Berechnete Gesamtbeträge / Remanet als total) -->
        <xsl:apply-templates mode="accounts" select="//tei:text/*[matches(@ana,'#bs_Remanet')]"/>
    </xsl:template>
    <xsl:template match="tei:category" mode="accounts">
            ...
            <xsl:variable name="levelup">
                <xsl:choose>
                    <xsl:when test="parent::tei:category">
                        <xsl:value-of select="concat($xmlBaseAccounts,'#',parent::tei:category/@xml:id)" />
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <xsl:apply-templates select="//tei:*[matches(@ana,current()/@xml:id)]/*[matches(@ana,'#bk_total') and not(matches(@ana,'#bk_page'))]">
            <xsl:with-param name="konto">
                <xsl:value-of select="concat($xmlBaseAccounts,'#',@xml:id)" />
            </xsl:with-param>
        </xsl:apply-templates>
        <xsl:apply-templates mode="accounts" select="tei:category">
            ...
            <bk:amount>
                <rdf:Description>
                    ...
                    <bk:num rdf:datatype="http://www.w3.org/2001/XMLSchema#decimal">
                        <xsl:value-of select="string(format-number(sum($betraege/*/number(text())),'#0'))" />
                    </bk:num>
