...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template match="tei:TEI" mode="debug">
        <div class="ym-wrapper">
            <div class="ym-wbox">
                <!-- Der Header sollte natürlich auch Daten aus dem Projekt aufnehmen -->
                <div class="wbox header">
                    <h1 style="color:white;">
                        <a href="{$cid/home}" style="color:white;">&lt; <xsl:value-of select="$cid/name"/>&gt;</a><br/>
                            <xsl:value-of
        ...
        <li>
            <a href="#{$id}">
                <xsl:if test="$context and matches(.,$context)">
                    <xsl:attribute name="style">background-color:#FFFF77</xsl:attribute>
                </xsl:if>
    ...
    <xsl:template mode="debug" match="tei:p|tei:fw|tei:closer">
        <a>
    ...
    <xsl:template name="betrag">
        <xsl:choose>
            <xsl:when test="(not(./@quantity) or ./@quantity = 0)">
                ...
                <xsl:value-of
                    select="sum(bk:umrechnung(xs:decimal(./tei:num/@value),./@unit))
                    + sum(sum(.//tei:measure[./@type='currency']/bk:umrechnung(xs:decimal(tei:num/@value),@unit)))
                    +     sum(.//tei:measure[./@type='currency' and ./@quantity]/bk:umrechnung(xs:decimal(@quantity), @unit))
                    +  sum(.//tei:measure[string(number(substring-before(text()[1],' '))) != 'NaN']/bk:umrechnung(xs:decimal(substring-before(text()[1],' ')), ./@unit))
                    "
                />
            </xsl:when>
            <xsl:otherwise>
