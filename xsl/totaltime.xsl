...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:bk="http://gams.uni-graz.at/rem/bookkeeping/" xmlns:sr="http://www.w3.org/2001/sw/DataAccess/rf1/result" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
    ...
    <xsl:param name="mode" />
    <xsl:variable name="gesuchteskonto" select="distinct-values(//sr:result/sr:superkonto/@uri)[1]" />
    <xsl:template match="/">
        ...
        <html>
            <head>
                <title>Zeitreihe Kontensummen</title>
                    ...
                    <xsl:for-each select="tokenize(//sr:result[1]/sr:pfad[1],'/')">
                        <xsl:if test="not(.='')">
                            <a href="./get?params=$1|&lt;http://gams.uni-graz.at/rem/%23{.}&gt;">
                                <xsl:value-of select="." />
                                [<a href="/archive/objects/query:rem-account-total/methods/sdef:Query/get?params=$1|&lt;http://gams.uni-graz.at/rem/%23{.}&gt;">Nur dieses Konto</a>]                            </a>
                        </xsl:if> / 
                    </xsl:for-each>
                </p>
                <xsl:choose>
...
<!--                                <tr class="gesamt">
                                    ...
                                    <xsl:for-each-group group-by="sr:o/@uri" select="//sr:result">
                                        <td>
                                            <xsl:variable name="totalsup">
                                                <xsl:choose>
                                                    <xsl:when test="sr:totalsup">
                                                        <xsl:value-of select="sr:totalsup" />
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:value-of select="sum(current-group()/xs:integer(sr:total))" />
                                                    </xsl:otherwise>
                                                </xsl:choose>
                                            </xsl:variable>
                                            <xsl:value-of select="format-number($totalsup div 240,'###.##0,00', 'european')" /> lb (<xsl:value-of select="format-number(sum(current-group()/xs:integer(sr:total)) div 240,'###.##0,00', 'european')" />)</td>
